...
interface Action {
  ...
  interface Props{
    route : RouteProp<ShopStackParamList,"Register">
    navigation:NativeStackScreenProps<ShopStackParamList,"Register">;    
}




const SellRegister = ({navigation,route}:Props) =>{   
    ...
    const removeItem = (idx:number)=>{
      let tmp = response.splice(idx,1);
      Alert.alert('info',`index ${idx}`)
      setResponse(()=>[...tmp]);
    }
    ...
    return (
        ...
        <View style={styles.buttonContainer} >
            ...
            buttonStyle={{...styles.buttonStyle,backgroundColor:'lightblue'}}
                title="SUBMIT"
                type="outline"
                onPress={()=> Alert.alert('info','TBD')} //postData()}
                />
        ...
        <View style={styles.inputContainer}>
          ...
          <View style={styles.imageAddContainer}>
              ...
              <FlatList
                      horizontal={true}
                      data={response}
                      renderItem={({index,item})=><>{(item!==null && item!==undefined) && item?.assets.map(({uri}) => (
                          <View key={uri} style={{...styles.image,margin:5}}>
                            <Image
                              ...
                              source={{uri: uri}}
                            />
                            <Button onPress={()=>removeItem(index)} style={{alignItems:'center',width:30}}>X</Button>
                          </View>))}</>}
